...
public class Shake : MonoBehaviour
{
   ...
   public Transform shakeCamera;
    ...
    public IEnumerator ShakeCamera(float duration = 0.07f, float magnitudePos = 0.3f, float magnitudeRot = 0.5f)
    {
        //지나간 시간을 누적할 변수
        float passTime = 0.0f;
        ...
        while (passTime < duration)
        {
            ...
            if (shakeRotate)
            {
                ...
                Vector3 shakeRot = new Vector3(Mathf.PerlinNoise(Time.time * magnitudeRot, 0.0f), 0, 0);
                //카메라의 회전값을 변경
